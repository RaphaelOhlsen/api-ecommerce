...
class UsuarioController {
  ...
  createRecovery(req, res, next) {
    ...
    if(!email) {
      return res.render('recovery', 
        { error: 'Preencha com o seu email', success: null}
      );
    }
    ...
    Usuario.findOne({ email }).then((usuario) => {
      if(!usuario) {
        return res.render('recovery',
          { error: 'Não existe usuário com este email', success: null}
        );
      }
  ...
  showCompleteRecovery(req, res, next) {
    if(!req.query.token) {
      return res.render('recovery',
        {error: 'Token não identificado', success: null}
      );
    }
    Usuario.findOne({ 'recovery.token': req.query.token }).then(usuario => {
      if(!usuario) {
        return res.render('recovery',
          { error: 'Não existe usuário com este token', success: null}
        );
      }
      if( new Date(usuario.recovery.date) < new Date() ) {
        return res.render('recovery', 
          { error: 'Token expirado. Tente novamente', success: null }
        );
      }
      return res.render('recovery/store', 
        { error: null, success: null, token: req.query.token }
      );
    }).catch(next);
  }
  ...
  completeRecovery(req, res, next) {
    ...
    if(!token || !password) {
      return res.render('recovery/store',
        { error: 'Preencha novamente com a sua nova senha', success: null , token: token }
      );
    }
    Usuario.findOne( {'recovery.token': token }).then(usuario => {
      if(!usuario) {
        return res.render('recovery', 
          { error: "Usuario não identificado", success: null }
        );
      }
